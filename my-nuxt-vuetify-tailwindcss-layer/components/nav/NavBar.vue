...
<template>
  <div class="px-4">
    <nav class="h-full navbar-grid py-4">
      ...
      <div data-pg-name="Hamburger" class="md:hidden">
        <NavHamburger @click="isMobileMenuOpen = true"></NavHamburger>
      </div>
      <div data-pg-name="NavPrimary" style="grid-area: primary-nav" class="hidden md:flex">
        <NavPrimary class="md:w-full" />
      </div>
      <div data-pg-name="Searchbox" style="grid-area: search" class="flex items-center">
        <v-form class="w-full">
          <v-text-field label="Search..." append-inner-icon="i-material-symbols-search-rounded" hide-details
            variant="outlined" class="w-full"></v-text-field>
        </v-form>
      </div>
      <div data-pg-name="Profile" class="flex justify-end space-x-2 md:items-center">
        ...
        <v-menu>
          <template #activator="{ props }">
            <v-btn text="Click me" :icon="true" v-bind="props" variant="tonal" class="md:ml-2" aria-label="Avatar">
              <v-avatar alt="Avatar" image="https://avatars.githubusercontent.com/u/73772701?v=4"
                size="large"></v-avatar>
            </v-btn>
          </template>
